...
## D-065 — Three-branch git pipeline mapped to Vercel environments

**Date:** 2026-05-28

**Decision:** Adopt a three-branch flow on GitHub (`development`, `main`, `production`) mapped 1-to-1 to Vercel environments:

| Branch | Vercel environment | Purpose |
|---|---|---|
| `development` | Development (CLI / localhost) | Active feature work; no auto-deploy needed |
...
Promotion is by merge (`development → main → production`). The Vercel Production Branch setting (Settings → Environments → Production → Branch Tracking) is set to `production`, so only pushes to `production` deploy to the live environment.

**Why:** Three clean environments without paying for a separate Supabase staging project. The same Supabase database is shared across environments, with safety enforced by `NEXT_PUBLIC_DEMO_MODE` per-environment (see D-067), not by separate URLs.

**Trade-off:** All three environments hit the same Supabase project, so a careless write from preview or localhost could touch production data. The `NEXT_PUBLIC_DEMO_MODE` gate prevents this — but the cost of full isolation later (separate Supabase project for staging) is real if test traffic ever grows.

---

## D-066 — `data/optimised-images.json` is a tracked build-time dependency

**Date:** 2026-05-28

**Decision:** Commit `data/optimised-images.json` to the repository. It was previously gitignored as a generated artefact, but `src/features/products/source.ts` imports it at build time for the demo-mode image fallback. Without it in the repo, Vercel builds fail with `Module not found: Can't resolve '../../../data/optimised-images.json'`.

The `.gitignore` carries a comment explaining the exception so a future contributor does not re-add the ignore rule:

```
data/scraped-images.json
data/scraped-products.json
# data/optimised-images.json is imported by src/features/products/source.ts
# at build time (demo-mode image fallback), so it must be committed.
```

**Why:** Build-time runtime dependencies (whether code, JSON, or assets) must be in the repository. Gitignoring something that is imported by source breaks every fresh clone, which is exactly what Vercel does on every deploy.

**Trade-off:** The file is regenerated by `npm run optimise` (~24 KB now, larger if more product images are added). Re-running the optimiser will produce a diff to commit — that is the cost of treating it as a tracked output.

---

## D-067 — `NEXT_PUBLIC_DEMO_MODE` is set per-environment, not per-branch

**Date:** 2026-05-28

**Decision:** Set `NEXT_PUBLIC_DEMO_MODE=true` (or leave unset, which defaults to `true`) on the **Preview** and **Development** Vercel environments. Set `NEXT_PUBLIC_DEMO_MODE=false` on the **Production** environment only. This is configured in Vercel via per-environment env-var scopes.

In code, the gate `useLocalOnly = isDemoMode || !hasSupabase` (in `src/features/products/source.ts`) and the matching gate in every server action means:

- **Demo mode ON** → reads come from bundled JSON; server actions never write to Supabase. Submissions log only a reference id.
- **Demo mode OFF** → reads and writes hit the live Supabase project, and bookings flow to the shared mobile-app database.

**Why:** All three Vercel environments share one Supabase project (D-065). Without per-environment demo-mode gating, a test booking from localhost or the preview URL would create a real row in production data. The `DEMO_MODE` flag is the single switch that draws the production / non-production boundary.

**Trade-off:** Anyone changing the Production Vercel env var to `true` (or unsetting it) silently puts the live site into demo mode. Mitigation: the value of `NEXT_PUBLIC_DEMO_MODE` on the Production environment should be reviewed on every domain cut-over and after every team-member change.

---

## D-068 — Hero `objectPosition` prop + cart CTA Button consistency

**Date:** 2026-05-28

**Decision:** Two small consistency fixes captured together:

1. **`Hero` supports an optional `objectPosition` prop** (`HeroImage.objectPosition?: string`, a Tailwind class such as `object-top`). The `<Image>` className is composed as `cn('object-cover hero-ken-burns', image.objectPosition ?? 'object-center')`. Used on `/about` so the founder photo is not cropped at the face.

2. **`CartDrawer` CTAs use the shared `Button` component** rather than hand-rolled `<Link>` elements. A raw `<Link>` with `bg-rose text-ivory text-small` was rendering text in the wrong colour (the cascade of `text-small` + `text-ivory` did not behave the same as the proven `Button` primary variant). Using `<Button variant="primary" fullWidth>` and `<Button variant="secondary" fullWidth>` guarantees the same look as every other CTA on the site.

**Why:** Every reusable visual control should go through the one component that gets it right, not a near-copy. The cart drawer was the last hand-rolled button surface; this brings it in line.

**Trade-off:** The Button component's `text-base` is slightly larger than the drawer's previous `text-small`, but the drawer is narrow and the larger text reads cleaner.
